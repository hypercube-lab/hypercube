...
pub fn receiver(
    ...
    Builder::new()
        .name("hypercube-receiver".to_string())
        .spawn(move || {
...
pub fn responder(name: &'static str, sock: Arc<UdpSocket>, r: BlobReceiver) -> JoinHandle<()> {
    Builder::new()
        .name(format!("hypercube-responder-{}", name))
        .spawn(move || loop {
...
//We would need to put a block of authentication in place before we can build the window, which is a TODO.
...
pub fn blob_receiver(sock: Arc<UdpSocket>, exit: Arc<AtomicBool>, s: BlobSender) -> JoinHandle<()> {
    //DOCUMENTED SIDE-EFFECT /Timeout on socket read of one second
    let timer = Duration::new(1, 0);
    sock.set_read_timeout(Some(timer))
        .expect("set socket timeout");
    Builder::new()
        .name("hypercube-blob_receiver".to_string())
        .spawn(move || loop {
            if exit.load(Ordering::Relaxed) {
                break;
            }
