...
mod tests {
        ...
        (0..num)
            .map(|_| {
                Entry::new_mut(
                    &mut id,
                    &mut num_hashes,
                    vec![Transaction::fin_plan_new_timestamp(
                        &keypair,
                        keypair.pubkey(),
                        keypair.pubkey(),
                        Utc::now(),
    ...
    fn make_test_entries() -> Vec<Entry> {
        ...
        let one = hash(&zero.as_ref());
        let keypair = Keypair::new();
        let tx0 = Transaction::fin_plan_new_vote(
            &keypair,
            Vote {
                version: 0,
        ...
        let tx1 = Transaction::fin_plan_new_timestamp(
            &keypair,
            keypair.pubkey(),
            keypair.pubkey(),
            Utc::now(),
    ...
    fn test_next_entries() {
        ...
        let keypair = Keypair::new();
        let tx_small = Transaction::fin_plan_new_vote(
            &keypair,
            Vote {
                version: 0,
        ...
        let tx_large = Transaction::fin_plan_new(&keypair, keypair.pubkey(), 1, next_id);
