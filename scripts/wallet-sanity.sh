...
check_balance_output() {
  ...
  exec 42>&1
  output=$($xpz_wallet "${entrypoint[@]}" balance | tee >(cat - >&42))
...
pay_and_confirm() {
  exec 42>&1
  signature=$($xpz_wallet "${entrypoint[@]}" pay "$@" | tee >(cat - >&42))
  $xpz_wallet "${entrypoint[@]}" confirm "$signature"
}

$xpz_keygen
$xpz_wallet "${entrypoint[@]}" address
check_balance_output "No account found" "Your balance is: 0"
$xpz_wallet "${entrypoint[@]}" airdrop 60
check_balance_output "Your balance is: 60"
$xpz_wallet "${entrypoint[@]}" airdrop 40
