...
(
  ...
  $xpz_bench_tps --identity $client_id --num-nodes 3 --reject-extra-nodes --converge-only
  rm -rf $client_id
) || flag_error
...
(
  source multinode-demo/common.sh
  set -x
  cp -R "$XPZ_CONFIG_DIR"/ledger /tmp/ledger-$$
  $xpz_ledger_tool --ledger /tmp/ledger-$$ verify
  rm -rf /tmp/ledger-$$
) || flag_error
