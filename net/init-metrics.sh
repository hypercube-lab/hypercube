...
Creates a testnet dev metrics database
  ...
  -e              Assume database already exists and XPZ_METRICS_CONFIG is
...
if $useEnv; then
  [[ -n $XPZ_METRICS_CONFIG ]] ||
    usage "Error: XPZ_METRICS_CONFIG is not defined in the environment"
else
  username=$1
  ...
  query() {
    echo "$*"
    curl -XPOST \
      "https://metrics.hypercube-lab.org:8086/query?u=${username}&p=${password}" \
      --data-urlencode "q=$*"
  }
  ...
  XPZ_METRICS_CONFIG="db=$netBasename,u=scratch_writer,p=topsecret"
fi

echo "export XPZ_METRICS_CONFIG=\"$XPZ_METRICS_CONFIG\"" >> "$configFile"
