...
snap)
  ...
  commonNodeConfig="\
    ...
    metrics-config=$XPZ_METRICS_CONFIG \
    rust-log=$RUST_LOG \
  ...
  if [[ $nodeType = leader ]]; then
    ...
    ln -sf -T /var/snap/hypercube/current/leader/current leader.log
    ln -sf -T /var/snap/hypercube/current/drone/current drone.log
  else
    nodeConfig="mode=validator $commonNodeConfig"
    ln -sf -T /var/snap/hypercube/current/validator/current validator.log
  fi

  logmarker="hypercube deploy $(date)/$RANDOM"
  logger "$logmarker"

  # shellcheck disable=SC2086 # Don't want to double quote "$nodeConfig"
  sudo snap set hypercube $nodeConfig
  snap info hypercube
  sudo snap get hypercube
...
local)
  ...
  validator)
    ...
    ./multinode-demo/validator.sh "$entrypointIp":~/hypercube "$entrypointIp:8001" >validator.log 2>&1 &
    ;;
  *)
