...
snap)
  ...
  xpz_ledger_tool=hypercube.ledger-tool
  xpz_keygen=hypercube.keygen

  ledger=/var/snap/hypercube/current/config/ledger
  client_id=~/snap/hypercube/current/config/client-id.json

  ;;
local)
  PATH="$HOME"/.cargo/bin:"$PATH"
  export USE_INSTALL=1
  entrypointRsyncUrl="$entrypointIp:~/hypercube"
...
if $ledgerVerify; then
  if [[ -d $ledger ]]; then
    (
      ...
      time $xpz_ledger_tool --ledger /var/tmp/ledger-verify verify
    )
  else
    echo "^^^ +++"
