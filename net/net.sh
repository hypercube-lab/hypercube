...
build() {
  ...
  if [[ $(uname) != Linux ]]; then
    MAYBE_DOCKER="ci/docker-run.sh hypercubelabs/rust"
  fi
  SECONDS=0
  (
    cd "$XPZ_ROOT"
...
startCommon() {
  declare ipAddress=$1
  test -d "$XPZ_ROOT"
  ssh "${sshOptions[@]}" "$ipAddress" "mkdir -p ~/hypercube ~/.cargo/bin"
  rsync -vPrc -e "ssh ${sshOptions[*]}" \
    "$XPZ_ROOT"/{fetch-perf-libs.sh,scripts,net,multinode-demo} \
    "$ipAddress":~/hypercube/
}
...
startLeader() {
  ...
  (
    ...
    snap)
      rsync -vPrc -e "ssh ${sshOptions[*]}" "$snapFilename" "$ipAddress:~/hypercube/hypercube.snap"
      ;;
    local)
      rsync -vPrc -e "ssh ${sshOptions[*]}" "$XPZ_ROOT"/farf/bin/* "$ipAddress:~/.cargo/bin/"
      ;;
    *)
    ...
    ssh "${sshOptions[@]}" -n "$ipAddress" \
      "./hypercube/net/remote/remote-node.sh $deployMethod leader $publicNetwork $entrypointIp $expectedNodeCount \"$RUST_LOG\""
  ) >> "$logFile" 2>&1 || {
    cat "$logFile"
    echo "^^^ +++"
    exit 1
...
startValidator() {
  ...
  (
    ...
    ssh "${sshOptions[@]}" -n "$ipAddress" \
      "./hypercube/net/remote/remote-node.sh $deployMethod validator $publicNetwork $entrypointIp $expectedNodeCount \"$RUST_LOG\""
  ) >> "$logFile" 2>&1 &
  declare pid=$!
...
startClient() {
  ...
  (
    ...
    ssh "${sshOptions[@]}" -f "$ipAddress" \
      "./hypercube/net/remote/remote-client.sh $deployMethod $entrypointIp $expectedNodeCount \"$RUST_LOG\""
  ) >> "$logFile" 2>&1 || {
    cat "$logFile"
    echo "^^^ +++"
    exit 1
...
sanity() {
  ...
  (
    ...
    ssh "${sshOptions[@]}" "$leaderIp" \
      "./hypercube/net/remote/remote-sanity.sh $sanityExtraArgs"
  ) || ok=false
...
start() {
  ...
  snap)
    if [[ -n $snapChannel ]]; then
      rm -f "$XPZ_ROOT"/xpz_*.snap
      if [[ $(uname) != Linux ]]; then
        (
          set -x
          XPZ_DOCKER_RUN_NOSETUID=1 "$XPZ_ROOT"/ci/docker-run.sh ubuntu:18.04 bash -c "
            set -ex;
            apt-get -qq update;
            apt-get -qq -y install snapd;
            snap download --channel=$snapChannel hypercube;
          "
        )
      else
        (
      ...
      snapFilename="$(echo "$XPZ_ROOT"/xpz_*.snap)"
      [[ -r $snapFilename ]] || {
  ...
  *)
  ...
  if [[ $deployMethod = "snap" ]]; then
    ...
    IFS=\  read -r _ networkVersion _ < <(
      ssh "${sshOptions[@]}" "$leaderIp" \
        "snap info hypercube | grep \"^installed:\""
    )
...
stopNode() {
  ...
  (
    ...
    ssh "${sshOptions[@]}" "$ipAddress" "
      set -x
      if snap list hypercube; then
        sudo snap set hypercube mode=
        sudo snap remove hypercube
      fi
      ! tmux list-sessions || tmux kill-session
      for pattern in hypercube- remote- oom-monitor net-stats; do
        pkill -9 \$pattern
      done
    "
  ) || true
