...
EOF
  ...
  (
    ...
    if [[ ! -r $sshPrivateKey ]]; then
      ...
      set -x -o pipefail
      for i in $(seq 1 30); do
        if cloud_FetchFile "$leaderName" "$leaderIp" /hypercube-id_ecdsa "$sshPrivateKey"; then
          break
        fi
...
# Place the generated private key at /hypercube-id_ecdsa so it's retrievable by anybody
# who is able to log into this machine
cat > /hypercube-id_ecdsa <<EOK
$(cat "$sshPrivateKey")
EOK
cat > /hypercube-id_ecdsa.pub <<EOK
$(cat "$sshPrivateKey.pub")
EOK
chmod 444 /hypercube-id_ecdsa
...
$(
  ...
  cat \
    ...
    create-hypercube-user.sh \
    add-hypercube-user-authorized_keys.sh \
    install-earlyoom.sh \
